...
public class SudokuLeetcode37 {
    ...
    static void solveSudoku(char[][] table) {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                char ch = table[i][j];
                if (ch != '.') { // 初始化冲突状态
                    ca[i][ch - '1'] = true; // 第i行数字5已经被占用
                    cb[j][ch - '1'] = true;
                    cc[i / 3 * 3 + j / 3][ch - '1'] = true; // 记录九宫格冲突
                }
            }
        }
    ...
    static boolean dfs(int i, int j, char[][] table, boolean[][] ca,
