...
//        merge(a1, 0, 2, 3, 6, a2);
    ...
    public static void mergeRecursive(int[] a1, int i, int iEnd, int j, int jEnd,
                              int[] a2, int k) {
        if (i > iEnd) {
            System.arraycopy(a1, j, a2, k, jEnd - j + 1);
            return;
        ...
        if (a1[i] < a1[j]) {
            a2[k] = a1[i];
            mergeRecursive(a1, i + 1, iEnd, j, jEnd, a2, k + 1);
        } else {
            a2[k] = a1[j];
            mergeRecursive(a1, i, iEnd, j + 1, jEnd, a2, k + 1);
        }
    }
    ...
    public static void mergeNotRecursive(int[] a1, int i, int iEnd, int j, int jEnd,
                             int[] a2) {
        int k = 0;
        while (i <= iEnd && j <= jEnd) {
            if (a1[i] < a1[j]) {
