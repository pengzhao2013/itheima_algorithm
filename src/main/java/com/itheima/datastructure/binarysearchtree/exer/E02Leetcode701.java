...
public class E02Leetcode701 {
    ...
    public TreeNode insertIntoBSTRecursion1(TreeNode node, int val) {
        if (node == null) {
            return new TreeNode(val);
        }
        if (node.val < val) {
            node.right = insertIntoBSTRecursion1(node.right, val);
        } else {
            node.left = insertIntoBSTRecursion1(node.left, val);
        }
        return node;
    }
    ...
    public TreeNode insertIntoBSTNotRecursion1(TreeNode root, int val) {
        TreeNode p = root;
        TreeNode parent = null;
        while (p != null) {
            if (p.val < val) {
