...
public class TestThreadUnsafe {
    ...
    Condition tailWaits = lock.newCondition(); // 条件变量对象 集合 可以存储多个进行阻塞的线程

    public void offer(String e) throws InterruptedException {
//        lock.lock(); // 加锁
        lock.lockInterruptibly(); // 加锁(可以在阻塞状态随时打断)
        try {
            while (isFull()) { //if -> 改成 while
                // 满了该做的事, offer 线程阻塞
                tailWaits.await();
                // 当前线程加入 tailWaits, 并且让此线程阻塞   唤醒 恢复运行：tailWaits.signal()
                // 被唤醒后不会再判断isFull了 唤醒后条件发生变化的情况叫虚假唤醒 唤醒后应重新检查条件
            }
            array[tail] = e;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        new Thread(()->{
            ...
            try {
                ...
                queue.tailWaits.signal(); //signal之前要先加锁
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
            } finally {
                queue.lock.unlock(); // 锁释放后再唤醒另一个线程
            }
        }, "t2").start();
    }
}
